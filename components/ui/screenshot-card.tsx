...
export default function ScreenshotCard({
  ...
  return (
    <>
      ...
      {isModalOpen && (
        <div 
          className={`fixed inset-0 z-50 flex items-center justify-center p-6 bg-black/90 backdrop-blur-sm transition-opacity duration-300 ${
            animationState === "enter" ? "opacity-0" : 
            animationState === "exit" ? "opacity-0" : "opacity-100"
          }`}
          onClick={closeModal}
        >
          <div 
            className={`relative max-w-7xl max-h-[95vh] w-full transition-all duration-300 ${
              animationState === "enter" ? "scale-95 opacity-0" : 
